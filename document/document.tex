...
\section{Literature review}
  ...
  \subsection{Project requirements}
    ...
    Existing research into the lower bounds of human arm accuracy shows that experienced surgeons have approximately \(5 \degree\) of vertical movement range on all joints when asked to hold their hand on a target, and close to \(3 \degree\) range for horizontal movement \cite{ref:movementrange}.
    However, the study is performed on novice and expert surgeons, which would imply that this range of joint movement is above average for most humans.
...
\section{Methods} \label{sec:methods}
  ...
  \subsection{Results of joint angle tracking methods}
    \textbf{This section is incomplete! The results from the marker tracking experiments will also be compiled here, and section 3.3 will discuss which method to use in section 4!}
    ...
    An interesting aspect that was encountered when performing the accuracy characterisation experiments with pose estimation is shown in \ref{fig:weezered}, and points to a downside not immediately apparent.
