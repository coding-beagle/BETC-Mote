...
\section{Introduction} % can use \input{} or \include{} for each section to draw content from other files rather than having everything in one big file
  ...
  To achieve this goal there will be an evaluation of available arm tracking methods, including:
  \begin{description}
    \item [\autoref{sec:litrev}] - Identifying and comparing existing technologies, including IMU based solutions discussed earlier, attitude and heading reference systems \cite{ref:Mazomenos_Biswas_Cranny_Rajan_Maharatna_Achner_Klemke_Jobges_Ortmann_Langendorfer_2016}, computer vision based systems \cite{ref:Phuong_Cong_2024}, and flex sensor based systems \cite{ref:Nalam_Manivannan_2014}.
    \item [\autoref{sec:methods}] - Developing a version of the tracking system suitable for controlling a simulation of the YuMi robot arm.
    \item [\autoref{sec:results}] - Devising experiments appropriate to comparing conventional methods of control to arm tracking teleoperation methods.
    \item [\autoref{sec:conc}] - Analysing the difference, if any, in performance between either method of control.
  \end{description}
...
\label{sec:litrev}
  ...
  \subsection{Requirements of a suitable tracking system}
    ...
    \begin{description}
      \item[Accuracy requirements] - Experienced surgeons have been measured to have \(5 \degree\) of vertical movement range on all joints when asked to hold their hand on a target, and close to \(3 \degree\) range for horizontal movement \cite{ref:movementrange}.
      ...
      \item[Latency requirements] - Teleoperation task performance generally degrades around the two key figures of \(600 \text{ ms}\) and \(1500 \text{ ms}\) \cite{ref:teleoplatency}.
  ...
  \subsection{Survey of existing tracking technologies}
    ...
    \begin{table}[h]
      ...
      IMU-based & \(3\) & \(4\) & \(3\) & \(5\) & \(15\) \\
       & \(100\text{ hours}\) inc PCB design + drift correction & £\(50-\text{£}60\) & \(2.8 \degree\) \cite{ref:imuerror} & <1 ms \cite{ref:imudatasheet} & \\
      \midrule
      ...
      % \midrule
      % AHRS & 3 & 3 & 4 & 5 & 15 \\
      %  & Moderate setup & £80-120 & 5-10° error & 100-200ms & \\
      \midrule
    ...
    The accuracy advantage of the marker tracking method makes it more appealing, though this is offset by the stupendous cost of conventional Vicon camera systems.

    Open source software along with ArUco markers could be used to offset this cost, which will make further analysis necessary before choosing a final method.
    ...
    These will be performed in the form of accuracy and latency experiments, which will give the freedom of being able to analyse more factors than the literature - including accuracy of measuring multiple joints angles simultaneously.
...
\label{sec:methods}
  ...
  \subsection{Comparison of both computer vision methods}
    ...
    The subject will move their arm repeatedly between these markers, and the recording is captured by a camera as described in fig \ref{fig:datacollection}.
    ...
    This secondary graph is then subtracted from the first graph to obtain the error value, from which we can calculate the root mean square error, and is shown in fig \ref{fig:elbow_flex_example}.
    
    \begin{figure}[h!]
      \centering
      \includegraphics[width=\linewidth]{images/elbow_flex_graph.png}
    ...
    This is clearly shown in fig \ref{fig:phantom_flex}.

    \begin{figure}[h!]
      \centering
      \includegraphics[scale=0.5]{images/elbow_flex_error.png}
      \caption{Top view of inferred \(3\)D joint positions.}
    ...
    A 7 DoF manipulator is created from the DH parameters in table \ref{tab:dh_parameters}.
  ...
  \subsection{Results of joint angle tracking methods}
    ...
    \begin{table}[h]
      ...
      \bottomrule
      \end{tabular}
      \caption{RMSE Error Comparison for Different Tracking Methods}
    ...
    Apart from the shoulder adduction/abduction measurements, each joint being tracked is close to the \(5 \degree\) requirements of the project.
...
\label{sec:experiments}
  ...
  The pose estimation algorithm shows \(60.3 \%\) reduction in time over the controller control in this task, with the noticeable difference being in the proportion of time spent in the 'approach' phase (see fig \ref{fig:experiment2res})
